...
class Session
{
   ...
   function login($subuser, $subpass, $subremember){
	...
	function fblogin($id){
      ...
      /* Checks that username is in database and password is correct */
      $subuser = stripslashes($subuser);
      $result = $database->confirmUserFB($id);
   ...
   function register($subuser, $subpass, $subemail){
	...
	function fbregister($fbid, $fbemail, $name){
	      global $database, $form, $mailer;  //The database, form and mailer object
		
		$form->num_errors=0;
		echo "<script>alert('Trying to register. ');</script>";
	      ...
	      $field = "user";  //Use field name for username
			...
			//removed for FB vers.
	         /* Check if username is already in use */
			if($database->emailTaken($fbemail)){
	      ...
	      /* Email error checking */	
	      $field = "email";  //Use field name for email
	      if(!$fbemail || strlen($fbemail = trim($fbemail)) == 0){
	      ...
	      /* No errors, add the new account to the */
		   	...
		   	echo "<script>alert('Trying to access DB');</script>";
		
	         if($database->addNewUserFB($fbid, $fbemail, $name)){
	            if(EMAIL_WELCOME){
	               $mailer->sendWelcome($subuser,$subemail,$subpass);
	            }
	            return 0;  //New user added succesfully
